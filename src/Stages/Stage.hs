...
instance Stage P.Precipitation where
  detectMe sol stp = P.SINGLEJET { P.nitrateSolution=sol, P.rate=RATE{amountAdded=0.0, overSeconds=0.0}, P.restingMinutes=0.0, P.temperature=Just 20 }

--if we've washed it and we're heating
instance Stage D.Digestion where
  detectMe sol stp = D.DIGESTION {D.restingMinutes=0.0, D.temperature=Just 20, D.additives=Nothing}