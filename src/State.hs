...
makeStates s st = groupBy equivalentStates $ scanl nextState firstState st
  where firstState = NOTHING { solution = s, additionalSolutions = [], temperature = Nothing, setPH = Nothing }

data State = NOTHING { solution :: S.Solution, additionalSolutions :: [S.Solution], setPH :: Maybe PowerHydrogen, temperature :: Maybe Temperature}
  |           NORMAL { solution :: S.Solution, givingSolution :: A.Addition, additionalSolutions :: [S.Solution], duration :: Maybe Minute, setPH :: Maybe PowerHydrogen, temperature :: Maybe Temperature }
  |          REVERSE { solution :: S.Solution, givingSolution :: A.Addition, additionalSolutions :: [S.Solution], duration :: Maybe Minute, setPH :: Maybe PowerHydrogen, temperature :: Maybe Temperature }
  |             NJET { solution :: S.Solution, givingSolutions :: [A.Addition], additionalSolutions :: [S.Solution], duration :: Maybe Minute, setPH :: Maybe PowerHydrogen, temperature :: Maybe Temperature }
  |      GENERICWASH { solution :: S.Solution, additionalSolutions :: [S.Solution], setPH :: Maybe PowerHydrogen }
  |        DIGESTION { solution :: S.Solution, additionalSolutions :: [S.Solution], duration :: Maybe Minute, setPH :: Maybe PowerHydrogen, temperature :: Maybe Temperature }
  deriving (Generic, Show, Eq, ToJSON, FromJSON)

equivalentStates :: State -> State -> Bool
equivalentStates (NOTHING sone _ _ _) (NOTHING stwo _ _ _) = sone == stwo
equivalentStates (NORMAL sone gone _ _ _ _) (NORMAL stwo gtwo _ _ _ _) = (sone == stwo) && (gone == gtwo)
equivalentStates (REVERSE sone gone _ _ _ _) (REVERSE stwo gtwo _ _ _ _) = (sone == stwo) && (gone == gtwo)
equivalentStates (NJET sone gones _ _ _ _) (NJET stwo gtwos _ _ _ _) = (sone == stwo) && (gones == gtwos)
equivalentStates (GENERICWASH sone _ _) (GENERICWASH stwo _ _) = sone == stwo
equivalentStates (DIGESTION sone _ _ _ _) (DIGESTION stwo _ _ _ _) = sone == stwo
...
nextState currentStage (TEMPERATURE setCelsius) = case currentStage of
                                                  w@(GENERICWASH s a _) -> DIGESTION { solution = S.washSolution $ mixStage w, additionalSolutions=[], duration = Nothing, setPH = Nothing, temperature = Just setCelsius}
...
nextState currentStage WASH = case currentStage of
                                  NOTHING {} -> currentStage
                                  w@(GENERICWASH s a _) -> GENERICWASH {solution = mixStage w, additionalSolutions=[], setPH = Nothing}
                                  d@(DIGESTION s _ _ _ _) -> GENERICWASH {solution =  mixStage d, additionalSolutions=[], setPH = Nothing}
                                  n@(NJET r g _ _ _ _) -> GENERICWASH {solution = mixStage n, additionalSolutions=[], setPH = Nothing}
                                  other -> GENERICWASH {solution = mixStage other, additionalSolutions=[], setPH = Nothing}
...
mixStage (NOTHING s a _ _) = foldl S.addSolutions s a
...
gelatinHalideRatio (S.SOLUTION salts sN (Just g) oC w (Just sH))
...
gelatinWaterRatio (S.SOLUTION salts sN (Just g) oC (Just water) sH)
